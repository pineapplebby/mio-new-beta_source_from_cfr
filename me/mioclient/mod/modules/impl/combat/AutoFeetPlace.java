...
extends Module {
    ...
    private EntityPlayer interceptedBy = null;
    ...
    public void onEnable() {
        ...
        if (!this.center.getValue().equals((Object)Center.NONE)) {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketBlockChange) {
            ...
            if (this.timing.getValue().equals((Object)Timing.SEQUENTIAL)) {
            ...
            if (event.getPacket() instanceof SPacketMultiBlockChange && this.timing.getValue().equals((Object)Timing.SEQUENTIAL)) {
                if (!this.lagTimer.passedMs(500L) && this.noLag.getValue().booleanValue()) {
                    return;
                }
    ...
    private void doFeetPlace() {
        if (AutoFeetPlace.mc.player.posY > this.startY && this.jumpDisable.getValue().booleanValue()) {
            this.disable();
            return;
        }
        if (!this.lagTimer.passedMs(500L) && this.noLag.getValue().booleanValue()) {
    ...
    private boolean isPlaceable(BlockPos pos) {
        ...
        if (!AutoFeetPlace.mc.world.getBlockState(pos).getMaterial().isReplaceable()) {
            placeable = false;
        }
