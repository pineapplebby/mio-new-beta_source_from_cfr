...
extends Module {
    ...
    private boolean check() {
        ...
        if (this.disable.getValue().booleanValue() && !this.startPos.equals((Object)EntityUtil.getRoundedBlockPos((Entity)WebTrap.mc.player))) {
            this.disable();
            return true;
        }
        if (obbySlot == -1) {
    ...
    private EntityPlayer getTarget(double range) {
        ...
        for (EntityPlayer player : WebTrap.mc.world.playerEntities) {
            if (EntityUtil.isntValid((Entity)player, range) || player.isInWeb || Managers.SPEED.getPlayerSpeed(player) > 30.0) continue;
            if (target == null) {
                target = player;
