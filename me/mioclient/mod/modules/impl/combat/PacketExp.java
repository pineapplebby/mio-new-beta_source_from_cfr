...
extends Module {
    ...
    @Override
    public void onTick() {
        if (!PacketExp.fullNullCheck() && this.mode.getValue().equals((Object)Mode.MIDDLECLICK) && Mouse.isButtonDown((int)2)) {
            this.throwExp();
        } else if (this.check() && this.mode.getValue().equals((Object)Mode.KEY) && Keyboard.isKeyDown((int)((Bind)this.bind.getValue()).getKey())) {
            this.enable();
            this.throwExp();
        }
        if (this.check() && this.mode.getValue().equals((Object)Mode.KEY) && !Keyboard.isKeyDown((int)((Bind)this.bind.getValue()).getKey())) {
            this.disable();
        }
    }
