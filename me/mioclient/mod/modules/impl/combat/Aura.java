...
extends Module {
    ...
    private void doAura() {
        ...
        if (this.targetMode.getValue() != TargetMode.FOCUS || target == null || Aura.mc.player.getDistanceSq(target) >= MathUtil.square(this.range.getValue().floatValue()) && (!this.teleport.getValue().booleanValue() || Aura.mc.player.getDistanceSq(target) >= MathUtil.square(this.teleportRange.getValue().floatValue())) || !Aura.mc.player.canEntityBeSeen(target) && !EntityUtil.canEntityFeetBeSeen(target) && Aura.mc.player.getDistanceSq(target) >= MathUtil.square(this.wallRange.getValue().floatValue()) && !this.teleport.getValue().booleanValue()) {
        ...
        if (this.teleport.getValue().booleanValue()) {
            Managers.POSITION.setPositionPacket(Aura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Aura.target.posY : Aura.target.posY + (double)target.getEyeHeight(), Aura.target.posZ, true, true, this.lagBack.getValue() == false);
        }
    ...
    private Entity getTarget() {
        ...
        for (Entity entity : Aura.mc.world.loadedEntityList) {
            if (!(this.players.getValue() != false && entity instanceof EntityPlayer || this.animals.getValue() != false && EntityUtil.isPassive(entity) || this.neutrals.getValue() != false && EntityUtil.isNeutralMob(entity) || this.hostiles.getValue() != false && EntityUtil.isMobAggressive(entity) || this.hostiles.getValue() != false && this.onlyGhasts.getValue() != false && entity instanceof EntityGhast || this.others.getValue() != false && EntityUtil.isVehicle(entity)) && (!this.projectiles.getValue().booleanValue() || !EntityUtil.isProjectile(entity)) || entity instanceof EntityLivingBase && EntityUtil.isntValid(entity, distance) || !this.teleport.getValue().booleanValue() && !Aura.mc.player.canEntityBeSeen(entity) && !EntityUtil.canEntityFeetBeSeen(entity) && Aura.mc.player.getDistanceSq(entity) > MathUtil.square(this.wallRange.getValue().floatValue()) || this.fovCheck.getValue().booleanValue() && !this.isInFov(entity, this.angle.getValue().intValue())) continue;
            if (target == null) {
                target = entity;
