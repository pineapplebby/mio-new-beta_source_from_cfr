...
extends Module {
    ...
    public void renderNameTag(EntityPlayer player, double x, double y, double z) {
        ...
        if (Chams.INSTANCE.isOn() && Chams.INSTANCE.sneak.getValue().booleanValue()) {
            preNameTag = new StringBuilder().append(isFriend ? "\u00a7" + (isFriend ? "b" : "c") : (player.isSneaking() ? "\u00a7r" : "\u00a7r")).append(this.getName(player)).append(this.gameMode.getValue() != false && info != null ? " [" + this.getGameModeShort(info.getGameType().getName()) + "]" : "").append(this.ping.getValue() != false && info != null ? " " + info.getResponseTime() + "ms" : "").append(this.health.getValue() != false ? " \u00a7" + this.getHealthColor(player.getHealth() + player.getAbsorptionAmount()) + MathHelper.ceil((float)(player.getHealth() + player.getAbsorptionAmount())) : "");
        }
        ...
        postNameTag = postNameTag.replace(".0", "");
        EntityPlayerSP renderPlayer = mc.getRenderViewEntity() == null ? NameTags.mc.player : (EntityPlayer)mc.getRenderViewEntity();
